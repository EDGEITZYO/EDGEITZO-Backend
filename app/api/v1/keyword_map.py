...
**앵커 지정 방법 두 가지. 둘 다 주면 `key`가 이기고, 둘 다 없으면 422입니다.**

- `keyword` — 사용자가 입력한 검색어. **키워드 탐색 화면의 기본 경로다.**
  완전 일치가 아니어도 된다: 어구 일치 → 단어 AND → 동의어 사전 → 임베딩(BGE-m3-ko) 순으로
  단계적으로 완화해 찾는다. 문장이 들어오면 명사를 뽑아 재시도한다.
  (실측: `면역력 강화`→면역력 증진, `기억력 감퇴`→기억, `치매 조기진단 연구 찾아줘`→치매)
  그래도 코퍼스에 없는 개념이면 404다 — 일시적 오류가 아니라 "그런 키워드가 없다"는 뜻이므로,
  화면에서 '재시도'로 안내하면 안 된다(재시도해도 같은 결과).

- `key` — 키워드 노드 key를 직접 지정. 노드가 확실할 때 문자열 매칭 단계를 건너뛴다.
  **AI 검색 결과에서 키워드맵으로 이어지는 동선이 있을 때** 채팅 응답의 `keyword_map_anchor.key`를
  그대로 넘기면 404 없이 그 결과와 맞는 그래프를 받는다. 그런 동선이 없으면 쓸 일이 없다.
  세션 재개(`GET /keyword-search/map/{user_id}`)의 `last_anchor_key`도 이 파라미터로 넘긴다.

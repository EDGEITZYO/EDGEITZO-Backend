...
class Settings(BaseSettings):
    ...
    # --- 해외 논문 → 코퍼스 연관 논문 (조회 시점 임베딩, 적재 없음) ---
    # 표본 120건 실측(2026-09-24). 정답 기준은 그 참고문헌을 인용한 코퍼스 논문의 키워드 겹침.
    #
    #   규칙                          반환   정밀도   평균건수  최대
    #   절대 0.52 + 상한 5            157건   46.5%    1.31      5
    #   절대 0.52, 상한 없음          233건   37.3%    1.94     20
    #   절대 0.52 + 1위대비 +0.03      96건   55.2%    0.80      5
    #   절대 0.52 + 1위대비 +0.05     154건   43.5%    1.28     18
    #   위 + 안전상한 10건            145건   44.1%    1.21     10
    #
    # 절대 임계값만으로는 부족하다. 같은 거리라도 1위가 0.40인 질의는 진짜 관련 논문이
    # 있는 경우라 2·3위도 쓸 만하지만, 1위가 0.54인 질의는 간신히 걸린 것이라 2위부터는
    # 대체로 무관하다. 그래서 1위와의 **상대 거리**로 한 번 더 자른다.
    #
    # 임계값을 넘는 게 없으면 빈 배열을 돌려준다. 코퍼스가 1,000편뿐이라 애초에 관련
    # 논문이 없는 해외 문헌이 절반이고(실측 52.5%), 그 상당수는 정부 연차보고서·교육과정
    # 문서·법령 조항이라 논문 자체가 아니다. 억지로 채우면 5건 중 2건이 무관해진다.
    #
    # 절대 임계값은 0.52에서 0.55로 올렸다. 0.52는 집계상으로는 좋아 보였지만
    # **경계에 실제 정답이 몰려 있었다**:
    #
    #   "Shelter location allocation model for flood evacuation planning"
    #     0.5207  민방위대피소 수용 능력 분석 및 최적 위치 선정   ← 0.52면 0.0007 차이로 잘린다
    #     0.5207  민방위 대피시설 위치 선정                      ← 같은 이유로 잘린다
    #
    # 0.52~0.55 구간의 정밀도는 약 44%로 확실한 구간(84%)보다 낮지만, 이런 정답이
    # 여기 있다. 반대로 보고서·법령류는 최고 거리가 0.62 이상이라 0.55로도 확실히 걸러진다.
    # 상대 밴드가 2차 필터로 남아 있어 1위가 0.55인 질의는 어차피 몇 건 안 붙는다.
    # distance를 응답에 내려주니 프런트에서 확신도를 달리 표기할 수도 있다.
    paper_citation_related_max_distance: float = 0.55
    paper_citation_related_relative_band: float = 0.05
    # 화면·페이로드 보호용 안전상한. 10건을 넘는 질의는 3.3%뿐이라 대부분 영향이 없다.
    paper_citation_related_limit: int = 10
    paper_citation_related_cache_ttl_seconds: int = 86400
